...
def create_widgets(master: tkinter.Frame) -> None:
    ...
    try:
        ...
        title_entry_label: tkinter.Label = get_label(
            master=master,
            text="Title*:",
        )
        ...
        title_entry_var: tkinter.StringVar = tkinter.StringVar()

        FORM_VARIABLES["title"] = {
            "variable": title_entry_var,
            "is_required": True,
        }

        title_entry: tkinter.Entry = get_entry(
            master=master,
            textvariable=title_entry_var,
        )
        ...
        body_text_label = get_label(
            master=master,
            text="Body Text*:",
        )

        body_text_label.grid(
            column=0,
            padx=5,
            pady=5,
            row=4,
        ...
        body_text_var: tkinter.StringVar = tkinter.StringVar()

        FORM_VARIABLES["text"] = {
            "variable": body_text_var,
            "is_required": True,
        }

        body_text_entry: dict[str, tkinter.Widget] = get_scrolled_text(master=master)

        body_text_entry["root"].grid(
            column=1,
            padx=5,
            pady=5,
            row=4,
        ...
        body_text_entry["text"].configure(height=50)

        body_text_entry["text"].bind(
            add="+",
            func=lambda event: body_text_var.set(
                value=body_text_entry["text"]
                .get(
                    index1="1.0",
                    index2="end-1c",
                )
