...
Date: 2025-12-13
Description: The note create form of the application.
"""
...
from models.observables import NoteObservableModel, ObservableModel
...
__all__: Final[list[str]] = ["get_note_create_form"]
...
_OBSERVABLE_MODEL: Optional[NoteObservableModel] = None
...
def _get_observable_model() -> NoteObservableModel:
    """
    Returns the observable model for the note create form.

    Args:
        None

    Returns:
        NoteObservableModel: The observable model for the note create form.
    """
...
def _set_observable_model(observable_model: NoteObservableModel) -> None:
    """
    Sets the observable model for the note create form.

    Args:
        observable_model (NoteObservableModel): The observable model for the note create form.
...
def _configure_grid() -> None:
    """
    Configures the grid of the note create form.
...
def _create_widgets() -> None:
    """
    Creates the note create form's widgets.
    ...
    _update_form(
        ...
        value={
            "is_required": True,
            "variable": _get_observable_model().title,
        },
    )

    ctk.CTkEntry(
        master=_get_master(),
        placeholder_text="Enter note title here...",
        textvariable=_get_form()["title"]["variable"],
    ).grid(
        column=1,
        padx=5,
        pady=5,
    ...
    _update_form(
        ...
        value={
            "is_required": True,
            "variable": _get_observable_model().text,
        },
    )

    text_textbox: ctk.CTkTextbox = ctk.CTkTextbox(
        height=800,
        master=_get_master(),
    )
...
def _on_destroy() -> None:
    """
    Handles the 'DESTROY_NOTE_CREATE_FORM' event.
...
def _subscribe_to_events() -> None:
    ...
    subscriptions: list[dict[str, Any]] = [
        ...
        {
            "event": DESTROY_NOTE_CREATE_FORM,
            "namespace": GLOBAL,
...
def _unsubscribe_from_events() -> None:
    ...
    log_info(message="Unsubscribed from all events for the note create form.")
...
def get_note_create_form(scrollable_frame: ctk.CTkScrollableFrame) -> None:
    """
    Gets the note create form.
    ...
    try:
        ...
        _set_observable_model(observable_model=NoteObservableModel())
        _clear_master()
        _create_widgets()
        _configure_grid()
        _subscribe_to_events()
    except Exception as e:
        log_error(message=f"Failed to get note create form: {e}")
        raise e
